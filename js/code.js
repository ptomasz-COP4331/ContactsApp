    ...
    xhr.onreadystatechange = function () {
      if (this.readyState === 4 && this.status === 200) {
        ...
        for (let i = 0; i < jsonObject.results.length; i++) {
          ...
          lContacts += `<td><button id='eb-${i}' onclick="editRow(${i})"><span class="material-symbols-rounded">edit</span><button id='sb-${i}' onclick="saveRow(${i})" display="none"><span class="material-symbols-rounded">save</span></button><button id='db-${i}' onclick="deleteRow(${i})"><span class="material-symbols-rounded">delete</span></button></td>`
          lContacts += '</tr>'
        }
