    ...
    xhr.onreadystatechange = function () {
      if (this.readyState === 4 && this.status === 200) {
        ...
        for (let i = 0; i < jsonObject.results.length; i++) {
          ...
          lContacts += `<td><button onclick="editRow(${i})"><span class="material-symbols-rounded">edit</span><button onclick="saveRow(${i})"><span class="material-symbols-rounded">save</span></button><button onclick="deleteRow(${i})"><span class="material-symbols-rounded">delete</span></button></td>`
          lContacts += '</tr>'
        }
